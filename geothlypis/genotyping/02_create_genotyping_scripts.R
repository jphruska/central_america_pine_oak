...
directory_name <- "11_geothlypis_genotype"
reference_genome_location <- "/lustre/scratch/johruska/central_america_pine_oak/geothlypis_ref/GCA_009764595.1_bGeoTri1.pri_genomic.fna"
cluster <- "nocona"
output_name <- "geothlypis_genotype"
popmap <- "geothlypis_popmap.txt"
individuals <- read.table(popmap, sep="\t")
faidx <- read.table("GCA_009764595.1_bGeoTri1.pri_genomic.fna.fai", stringsAsFactors=F)
singularity_bind <- "--bind /lustre/work/johruska:/lustre/work/johruska --bind /lustre/scratch/johruska:/lustre/scratch/johruska"
...
write("module load gcc/9.2.0", file=a.script, append=T)
write("module samtools/1.11", file=a.script, append=T)
write("", file=a.script, append=T)
...
gatk_command <- paste0("singularity exec ", singularity_bind, " $SINGULARITY_CACHEDIR/", name_of_gatk_singularity_image, ' gatk --java-options "-Xmx', total_mem1, 'g" HaplotypeCaller -R ', reference_genome_location, " -I ", a_name, " -ERC GVCF -O ", project_directory, "/02_vcf/", "${name_array}", "._${ind_array}_.g.vcf", " --QUIET --intervals ", "${chr_array}")
...
#gatk 4.2.3
gatk_command <- paste0("singularity exec ", singularity_bind, " $SINGULARITY_CACHEDIR/", name_of_gatk_singularity_image, ' gatk --java-options "-Xmx', total_mem2, 'g" GenomicsDBImport --genomicsdb-shared-posixfs-optimizations ', vcf_total, " --genomicsdb-workspace-path ", project_directory, "/02_vcf/", "${name_array}", " -L ", "${interval_array}")
